...
from scripts.collector_unidade_fornecimento import (
    collect_unidades_por_grupo_classe,
    collect_unidades_por_pdm,
    fetch_unidades,
)
...
def test_fetch_unidades_with_codigo_pdm():
    payload = {
        "resultado": [{"numeroSequencialUnidadeFornecimento": 339030, "siglaUnidadeFornecimento": "UN"}],
        "totalRegistros": 1,
        "paginasRestantes": 0,
    }
...
def test_fetch_unidades_200_empty_is_valid():
...
def test_fetch_unidades_429_with_retry_after_exhausted_raises():
...
def test_fetch_unidades_transient_then_success():
    ok = DummyHttpResponse({"resultado": [{"numeroSequencialUnidadeFornecimento": 1}], "paginasRestantes": 0})
    with patch("urllib.request.urlopen", side_effect=[_http_error(503), ok]):
        assert fetch_unidades(codigo_pdm=2640)["resultado"] == [{"numeroSequencialUnidadeFornecimento": 1}]


def test_fetch_unidades_timeout_raises():
    with patch("urllib.request.urlopen", side_effect=socket.timeout("timed out")):
        with pytest.raises(HttpFetchError):
            fetch_unidades(codigo_pdm=2640)


def test_fetch_unidades_invalid_json_raises():
...
def test_collect_por_pdm_invalid_envelope_raises(workdir):
    with patch("urllib.request.urlopen", return_value=DummyHttpResponse({"mensagem": "ok"})):
        with pytest.raises(InvalidEnvelopeError):
            collect_unidades_por_pdm(2640, resume=False)
...
def test_collect_iterates_official_pdms_from_e3(workdir):
    ...
    def fake_urlopen(req, *args, **kwargs):
        ...
        return DummyHttpResponse({"resultado": [{"numeroSequencialUnidadeFornecimento": 339030}], "paginasRestantes": 0})

    with patch("urllib.request.urlopen", side_effect=fake_urlopen):
        unidades = collect_unidades_por_grupo_classe(78, 7830, resume=False)
...
def test_collect_e3_without_pdms_for_group_is_valid_empty(workdir):
    ...
    with patch("urllib.request.urlopen") as mock_urlopen:
        assert collect_unidades_por_grupo_classe(78, 7830) == []
        assert not mock_urlopen.called


def test_collect_unidades_fails_on_502(workdir):
    with patch("urllib.request.urlopen", side_effect=_http_error(502)):
        with pytest.raises(HttpFetchError):
            collect_unidades_por_grupo_classe(78, 7830, pdms=[2640], resume=False)
...
def test_collect_unidades_partial_failure_and_resume(tmp_path):
    ...
    payload_p2 = {
        "resultado": [{"numeroSequencialUnidadeFornecimento": 339031, "siglaUnidadeFornecimento": "CX"}],
        "paginasRestantes": 0,
    }
    with patch("urllib.request.urlopen", return_value=DummyHttpResponse(payload_p2)):
        ...
        assert len(unidades) == 2
        assert unidades[0]["numeroSequencialUnidadeFornecimento"] == 339030
        assert unidades[1]["numeroSequencialUnidadeFornecimento"] == 339031
