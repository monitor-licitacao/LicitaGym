...
class AgenteJuridico:
    ...
    def analisar_documento(self, caminho: str) -> Dict:
        ...
        # Fazer parsing
        from ingestor import ParserDocumento, assert_safe_local_path

        try:
            caminho_seguro = assert_safe_local_path(caminho)
        ...
        if extensao == '.pdf':
            texto = ParserDocumento.parse_pdf(str(caminho_seguro))
        elif extensao == '.docx':
            texto = ParserDocumento.parse_docx(str(caminho_seguro))
        ...
        metadados = ExtratorMetadados.extrair(texto, str(caminho_seguro))
